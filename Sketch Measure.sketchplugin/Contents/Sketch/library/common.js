...
SM.extend({
	...
	colors: {
		...
		property: {
			layer: { r: 0.960784, g: 0.65098, b: 0.137255, a: 1 },
			text: { r: 1, g: 1, b: 1, a: 1 },
		},
		lite: {
			layer: { r: 0.564706, g: 0.07451, b: 0.996078, a: 1 },
			text: { r: 1, g: 1, b: 1, a: 1 },
		},
		note: {
...
SM.extend({
	...
	getRadius: function (layer) {
		if (layer.layers && this.is(layer.layers().firstObject(), MSRectangleShape)) {
			return layer.layers().firstObject().cornerRadiusString().split(";").map(Number).length == 1
				? layer.layers().firstObject().fixedRadius()
				: layer.layers().firstObject().cornerRadiusString().split(";").map(Number);
...
SM.extend({
	getImage: function (size, name) {
		...
		(suffix = isRetinaDisplay ? "@2x" : ""), (imageURL = NSURL.fileURLWithPath(this.pluginSketch + "/toolbar/" + name + suffix + ".png")), (image = NSImage.alloc().initWithContentsOfURL(imageURL));
	...
	Toolbar: function () {
		...
		if (!Toolbar) {
			...
			Toolbar.setBackgroundColor(NSColor.colorWithRed_green_blue_alpha(0.1, 0.1, 0.1, 1));
			...
			var contentView = Toolbar.contentView(),
				closeButton = self.addButton(NSMakeRect(14, 14, 20, 20), "icon-close", function (sender) {
					coscript.setShouldKeepAround(false);
					threadDictionary.removeObjectForKey(identifier);
					Toolbar.close();
				}),
				overlayButton = self.addButton(NSMakeRect(64, 14, 20, 20), "icon-overlay", function (sender) {
					self.updateContext();
					self.init(self.context, "mark-overlays");
				}),
				sizesButton = self.addButton(NSMakeRect(112, 14, 20, 20), "icon-sizes", function (sender) {
					self.updateContext();
				...
				spacingsButton = self.addButton(NSMakeRect(160, 14, 20, 20), "icon-spacings", function (sender) {
					self.updateContext();
					if (NSEvent.modifierFlags() == NSAlternateKeyMask) {
				...
				propertiesButton = self.addButton(NSMakeRect(208, 14, 20, 20), "icon-properties", function (sender) {
					self.updateContext();
					if (NSEvent.modifierFlags() == NSAlternateKeyMask) {
				...
				notesButton = self.addButton(NSMakeRect(258, 14, 20, 20), "icon-notes", function (sender) {
					self.updateContext();
					self.init(self.context, "mark-note");
				}),
				exportableButton = self.addButton(NSMakeRect(306, 14, 20, 20), "icon-slice", function (sender) {
					self.updateContext();
					if (NSEvent.modifierFlags() == NSAlternateKeyMask) {
				...
				colorsButton = self.addButton(NSMakeRect(354, 14, 20, 20), "icon-colors", function (sender) {
					self.updateContext();
					self.init(self.context, "color");
				}),
				exportButton = self.addButton(NSMakeRect(402, 14, 20, 20), "icon-export", function (sender) {
					self.updateContext();
					self.init(self.context, "export");
				}),
				hiddenButton = self.addButton(NSMakeRect(452, 14, 20, 20), "icon-hidden", function (sender) {
					self.updateContext();
					self.init(self.context, "hidden");
				}),
				lockedButton = self.addButton(NSMakeRect(500, 14, 20, 20), "icon-locked", function (sender) {
					self.updateContext();
					self.init(self.context, "locked");
				}),
				settingsButton = self.addButton(NSMakeRect(548, 14, 20, 20), "icon-settings", function (sender) {
					self.updateContext();
...
SM.extend({
	sizes: function (options) {
		...
		var length = type == "height" ? targetRect.height : targetRect.width,
			percentageType = byPercentage && type == "width" ? "width" : byPercentage && type == "height" ? "height" : undefined,
		...
		var tempRect = this.getRect(temp),
			...
			markPlacement =
				type == "height"
					? ruler.rect.height > tempRect.height + 28
						? "center"
						: placement == "right"
						? "right"
						: placement == "left"
						...
						? "right"
						: "left"
					: ruler.rect.width > tempRect.width + 28
					? "middle"
					: placement == "bottom"
					? "bottom"
					: placement == "top"
	...
	liteSizes: function () {
		...
		for (var i = 0; i < selection.count(); i++) {
			var target = selection[i],
				...
				widthPlacement = distance.top < distance.bottom ? "bottom" : distance.top == distance.bottom ? "middle" : "top",
				heightPlacement = distance.left > distance.right ? "left" : distance.left == distance.right ? "center" : "right";
	...
	importColors: function () {
		...
		colors.forEach(function (color) {
			if (
				...
				color.color.a !== null &&
				color.color.a !== undefined &&
				color.color.r !== null &&
				color.color.r !== undefined &&
				color.color.g !== null &&
				color.color.g !== undefined &&
				color.color.b !== null &&
				color.color.b !== undefined &&
				color.color["argb-hex"] !== null &&
				color.color["argb-hex"] !== undefined &&
				color.color["color-hex"] !== null &&
				color.color["color-hex"] !== undefined &&
				color.color["css-rgba"] !== null &&
				color.color["css-rgba"] !== undefined &&
				color.color["ui-color"] !== null &&
				color.color["ui-color"] !== undefined
			) {
	...
	makeExportable: function (optionKey) {
		...
		for (var i = 0; i < this.selection.count(); i++) {
			...
			if (!optionKey && !this.is(layer, MSSliceLayer)) {
				...
				var msRect = MSRect.rectWithUnionOfRects([MSRect.alloc().initWithRect(slice.absoluteRect().rect()), MSRect.alloc().initWithRect(layer.absoluteRect().rect())]);
...
SM.extend({
					...
					if (self.is(tempSymbolLayer, MSSymbolInstance)) {
						...
						if (overrides && overrides[symbolMasterObjectID] && !!overrides[symbolMasterObjectID].symbolID) {
							var changeSymbol = self.find(
								{ key: "(symbolID != NULL) && (symbolID == %@)", match: self.toJSString(overrides[symbolMasterObjectID].symbolID) },
	...
	getText: function (artboard, layer, layerData, data) {
		if (layerData.type == "text" && layer.attributedString().treeAsDictionary().value.attributes.length > 1) {
			...
			for (var a = 0; a < svgSpans.length; a++) {
				...
				offsetX = !offsetX || (offsetX && offsetX > this.toJSNumber(attrsData.x)) ? this.toJSNumber(attrsData.x) : offsetX;

				offsetY = !offsetY || (offsetY && offsetY > this.toJSNumber(attrsData.y)) ? this.toJSNumber(attrsData.y) : offsetY;
			...
			textData.forEach(function (tData) {
				if (tData["content"].trim() && (colorHex != tData.fill || Object.getOwnPropertyNames(tData).length >= 4)) {
					...
					self.getLayer(artboard, textLayer, data);
	...
	exportImage: function (options) {
		...
		savePathName.push(options.path, "/", options.prefix, options.name, options.suffix, ".", options.format);
		savePathName = savePathName.join("");
